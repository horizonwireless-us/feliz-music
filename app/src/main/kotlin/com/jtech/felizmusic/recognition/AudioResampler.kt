...
/**
 * Decoded PCM audio plus the metadata the fingerprinter needs to interpret it.
 *
 * Ported verbatim from Metrolist's recognition feature.
 */
...
/**
 * Linear-interpolation resampler. Resamples mono 16-bit PCM audio to the sample rate the Shazam
 * fingerprinter requires (16 kHz).
 */
object AudioResampler {
    ...
    suspend fun resample(
        ...
        try {
            Timber.tag(TAG).d(
                "Resampling: %dHz → %dHz, %d bytes input",
                decodedAudio.sampleRate,
                outputSampleRate,
                decodedAudio.data.size,
            )
            ...
            for (i in 0 until outputLength) {
                ...
                val sample = if (srcIndex + 1 < inputSamples.size) {
                    (inputSamples[srcIndex] * (1.0 - fraction) + inputSamples[srcIndex + 1] * fraction)
                        .toInt()
                        .toShort()
                } else {
            ...
            Timber.tag(TAG).d(
                "Resampling complete: %d bytes output, %dHz, %d channels",
                resampledData.size,
                outputSampleRate,
