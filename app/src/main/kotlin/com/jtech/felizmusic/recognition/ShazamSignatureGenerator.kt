 ...
 *
 * Ported from Metrolist with one change versus the original: it uses [java.util.Base64] instead of
 * `android.util.Base64`, so the generator is pure JVM and unit-testable without an Android runtime.
 * `java.util.Base64.getEncoder()` produces the same standard, unwrapped, padded output as
...
internal object ShazamSignatureGenerator {
    ...
    private class SignatureGeneratorState {
        ...
        private fun encodeSignature(): String {
            ...
            val base64 = Base64.getEncoder().encodeToString(fullBytes)
