...
/**
 * Server-listed, fine-grained content overrides: specific item ids that are hidden EVERYWHERE in the
 * app, for every user, when content filtering is enabled. This is the surgical complement to the
 * artist whitelist — a *mixed* channel/artist stays whitelisted while specific items from it are
 * hidden. `blockedContentIds` supports the single reason `global`; any legacy or unknown reason is
 * treated as `global` — the safe direction (over-block rather than leak).
 *
 ...
 * legitimate hits), an id override is surgical and safe to apply on BOTH engines, every surface.
 *
...
object BlockedIdsCache {
    ...
    fun isBlocked(id: String?, config: ContentFilterConfig): Boolean {
        ...
        // Global-only blocks: any recorded id is hidden for everyone when filters are enabled.
